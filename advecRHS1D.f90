...
do iface = 1, nface_global
    ...
    ! Numerical flux in the same orientation as flux from face 1
    do istate = 1, nstate
    do idir = 1, ndim
        if(global_faces(iface)%reversed) then
            flux_numerical(1:nnode_face, istate, idir, iface) = flux_numerical(1:nnode_face, istate, idir, iface) &
               + 0.5d0*( ffaces(1:nnode_face,istate,idir,iface1,iele1) &
                        +ffaces(nnode_face:1:-1,istate,idir,iface2,iele2) ) &
               + 0.5d0*wavespeed(idir)* &
                    ( ufaces(1:nnode_face,istate,iface1,iele1) &
            ...
            flux_numerical(1:nnode_face, istate, idir, iface) = flux_numerical(1:nnode_face, istate, idir, iface) &
               + 0.5d0*( ffaces(1:nnode_face,istate,idir,iface1,iele1) &
                        +ffaces(1:nnode_face,istate,idir,iface2,iele2) ) &
               + 0.5d0*wavespeed(idir)* &
                    ( ufaces(1:nnode_face,istate,iface1,iele1) &
                        * elements(iele1)%normals(1:nnode_face,iface1,idir) &
...
do iele = 1, nele
    ...
    do istate = 1, nstate
        ...
        do iface = 1, elements(iele)%nface
            ...
            !print*, 
            !print*, 'iele', iele, 'istate', istate, 'iface', iface
            !print*, 'dfn', df_n(1:nnode_face, istate, iface, iele)
            elements(iele)%rhs(:,istate) = elements(iele)%rhs(:,istate) &
                + matmul(Lift(1:nbasis, 1:nnode_face, iface), df_n(1:nnode_face, istate, iface, iele))
            !print*, iface
            !print*, matmul(Lift(1:nbasis, 1:nnode_face, iface), df_n(1:nnode_face, istate, iface, iele))
        end do
